...
func generateNinjaFiles(root string) error {
	...
	nf := &ninja_writer.NinjaFile{}
	nf.AddComment("Generated by cmd/bootstrap")
	nf.AddComment(fmt.Sprintf("Bootstrap epoch: %d", bootstrapEpoch))
	nf.AddVariable("builddir", filepath.Join(root, "build"))
	nf.AddVariable("ninja_stage1", filepath.Join(root, "build", "ninja_stage1", "ninja_mod"))
	nf.AddVariable("ninja_stage2", filepath.Join(root, "build", "ninja_stage2", "ninja_mod"))
	nf.AddVariable("ninja_monitor", filepath.Join(root, "build", "ninja_monitor"))
	nf.AddDefault("$ninja_monitor")

	return nf.WriteFile(filepath.Join(root, "build", "boot.ninja"))
}
