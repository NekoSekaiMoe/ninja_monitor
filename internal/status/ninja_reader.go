...
import (
	...
	"ninja_monitor/internal/ninja_frontend"
)
...
func (n *NinjaReader) Close() {
	...
	// Don't output errors for running actions when ninja exits -
	// ninja already outputs "build stopped: interrupted by user"
}
...
// HasAnyOutput returns true if any command run by ninja had any output to stdout/stderr. Be sure to only
